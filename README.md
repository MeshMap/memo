# SPL Spatial Memo Program

> **Note:** This is a fork of the [original SPL Memo Program](https://github.com/solana-program/memo) modified to support spatial content records.

<a href="https://github.com/solana-program/memo/actions/workflows/main.yml"><img src="https://img.shields.io/github/actions/workflow/status/solana-program/memo/main.yml?logo=GitHub" /></a>
<a href="https://crates.io/crates/spl-spatial-memo"><img src="https://img.shields.io/crates/v/spl-memo?logo=rust" /></a>
[![Docs.rs](https://docs.rs/spl-memo/badge.svg?logo=rust)]
<a href="https://crates.io/crates/spl-memo-client"><img src="https://img.shields.io/crates/v/spl-memo-client?logo=rust" /></a>
<a href="https://explorer.solana.com/address/SCRcKjZtuyQHCBTenEpSzKABTqSq5CaKpHRnRgDvgjV"><img src="https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fraw.githubusercontent.com%2Fsolana-program%2Fmemo%2Fmain%2Fprogram%2Fidl.json&query=%24.version&label=program&logo=data:image/svg%2bxml;base64,&color=9945FF" /></a>
<a href="https://www.npmjs.com/package/@solana-program/memo"><img src="https://img.shields.io/npm/v/%40solana-program%2Fmemo?logo=npm&color=377CC0" /></a>

A program that validates a string of UTF-8 encoded characters and logs it
in the transaction log, with a focus on spatial content records. The program also verifies that any accounts provided are
signers of the transaction, and if so, logs their addresses. It can be used to
record spatial data on-chain, stored in the instruction data of a successful
transaction, and optionally verify the originator.

## Modifications from Original SPL Memo

This fork includes the following modifications:
- Package name changed to `spl-spatial-memo`
- Program description updated to reference spatial content records
- New program ID: `SCRcKjZtuyQHCBTenEpSzKABTqSq5CaKpHRnRgDvgjV`

The original code was from the
[solana-program-library](https://github.com/solana-labs/solana-program-library/tree/master/memo)
and was later maintained in the [solana-program/memo](https://github.com/solana-program/memo) repo.

Full documentation is available at https://spl.solana.com/memo

## Audit

The [security-audits repo](https://github.com/solana-labs/security-audits) contains
all past and present program audits.

## Project setup

The first thing you'll want to do is install NPM dependencies which will allow you to access all the scripts and tools provided by this template.

```sh
pnpm install
```

## Program

The `program` folder in the root of this repository contains the program
implementation.

The following scripts build, test, format and lint the program respectively.

```sh
pnpm programs:build
pnpm programs:test
pnpm programs:format
pnpm programs:lint
```

## Generating IDLs

You may use the following command to generate the IDLs.

```sh
pnpm generate:idls
```

Note that, to ensure IDLs are generated using the correct framework version, the
specific version used by the program will be downloaded and used locally.

## Generating clients

Once the program's IDL has been generated, you can generate clients for them using the following command.

```sh
pnpm generate:clients
```

Alternatively, you can use the `generate` script to generate both the IDL and the clients at once.

```sh
pnpm generate
```

## Managing clients

The following clients are available for your programs. You may use the following links to learn more about each client.

- [JS client](./clients/js)
- [Rust client](./clients/rust)

## Starting and stopping the local validator

The following script is available to start your local validator.

```sh
pnpm validator:start
```

By default, if a local validator is already running, the script will be skipped. You may use the `validator:restart` script instead to force the validator to restart.

```sh
pnpm validator:restart
```

Finally, you may stop the local validator using the following command.

```sh
pnpm validator:stop
```
